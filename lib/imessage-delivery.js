...
export function messagesTransport(payload, { run = spawnSync, platform = process.platform } = {}) {
  ...
  if (result.error || result.status !== 0) {
    ...
    if (payload.mode === 'check') throw new Error(`Messages readiness check failed (${result.error?.code || `exit ${result.status}`}). Check macOS Automation permission and the signed-in Messages account. No message was sent.`)
  ...
  if (!response.ok && response.code === -1743) throw new Error('macOS denied Automation access to Messages. Allow it in System Settings > Privacy & Security > Automation, then run delivery-check again.')
