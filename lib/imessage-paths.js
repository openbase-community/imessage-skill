...
export function defaultRuntimeRoot() {
  return resolve(process.env.IMESSAGE_RUNTIME_HOME || join(homedir(), '.imessage-cli'))
}
...
export function ensureRuntimeLayout({ root = null } = {}) {
  ...
  const dirs = [
    ...
    [join(runtimeRoot, 'data', 'outbox'), 0o770],
    [join(runtimeRoot, 'data', 'protected'), 0o700],
    [join(runtimeRoot, 'data', 'protected', 'source'), 0o700],
    [join(runtimeRoot, 'data', 'protected', 'source', 'snapshots'), 0o700],
    [join(runtimeRoot, 'inbox'), 0o770],
...
function ensureDirectory(path, mode) {
  ...
  try {
    ...
    try {
      ...
      // The sudo installer owns authoritative mode repair.
    }
